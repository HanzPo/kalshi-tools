...
export function BetSlipPreview({ config }: BetSlipPreviewProps) {
  const calculatePayout = (wager: number, odds: number): number => {
    // Kalshi odds are in percentage, payout = wager / (odds/100)
  ...
  return (
    <div className="bet-slip-container">
      <div id="bet-slip-preview" className="bet-slip-preview">
        <div className="bet-slip-header">
          <a href="https://kalshi.com/?utm_source=kalshitools" target="_blank" rel="noopener noreferrer">
            <svg width="78" height="20" viewBox="0 0 78 20" fill="none" xmlns="http://www.w3.org/2000/svg" className="bet-slip-logo">
        ...
        <div className="bet-slip-content">
          <div className="bet-slip-question">
            <div>
              <h2>{config.title}</h2>
              <div className="bet-slip-answer">
                {config.answer}
              </div>
            </div>
            {config.image && (
              <div className="bet-slip-image-container">
                <img src={config.image} alt="Bet slip" className="bet-slip-image" />
              </div>
            )}
          </div>
          ...
          <div className="bet-slip-details">
            <div className="bet-slip-row">
              ...
              <span className="bet-slip-value">{config.odds}% chance</span>
            </div>
            <div className="bet-slip-row">
              <span className="bet-slip-label">Cost</span>
              <span className="bet-slip-value">${config.wager.toLocaleString()}</span>
            </div>
            <div className="bet-slip-row bet-slip-payout-row">
              <span className="bet-slip-label">Payout if win</span>
              <span className="bet-slip-payout">${payout.toLocaleString()}</span>
            </div>
          </div>
        </div>
